/**
 * Membership Preferences Repository
 *
 * ESSENTIAL MODULES INTEGRATION:
 * - errors: Uses ErrorCodes/ErrorMessages for structured error handling
 * - integrations: Uses DataverseService for all data operations
 * - interfaces: Implements MembershipPreferenceRepository contract
 * - mappers: Uses MembershipPreferenceMapper for data transformation
 *
 * SIMPLIFICATION PHILOSOPHY:
 * - Essential CRUD operations only
 * - Simple query patterns for OSOT membership preferences management needs
 * - Follows membership-settings repository pattern exactly
 * - Clean abstraction over DataverseService
 * - Structured error handling with proper logging
 * - Hard delete (no soft delete)
 */

import { Injectable } from '@nestjs/common';
import { DataverseService } from '../../../../integrations/dataverse.service';
import { ErrorCodes } from '../../../../common/errors/error-codes';
import { ErrorMessages } from '../../../../common/errors/error-messages';
import { MEMBERSHIP_PREFERENCES_ODATA } from '../constants/membership-preference.constants';
import { MembershipPreferenceRepository } from '../interfaces/membership-preference-repository.interface';
import { MembershipPreferenceInternal } from '../interfaces/membership-preference-internal.interface';
import { MembershipPreferenceDataverse } from '../interfaces/membership-preference-dataverse.interface';
import { MembershipPreferenceMapper } from '../mappers/membership-preference.mapper';
import { getAppForOperation } from '../../../../utils/dataverse-app.helper';

export const MEMBERSHIP_PREFERENCE_REPOSITORY =
  'MEMBERSHIP_PREFERENCE_REPOSITORY';

/**
 * DataverseMembershipPreferenceRepository
 *
 * Repository implementation for Membership Preferences entities using Microsoft Dataverse.
 * Provides a clean abstraction layer between the application and Dataverse API.
 */
@Injectable()
export class DataverseMembershipPreferenceRepository
  implements MembershipPreferenceRepository
{
  constructor(private readonly dataverseService: DataverseService) {}

  /**
   * Create a new membership preference record in Dataverse
   */
  async create(
    preferenceData: Partial<MembershipPreferenceInternal>,
  ): Promise<MembershipPreferenceInternal> {
    try {
      // Use main app credentials for create operations
      const app = getAppForOperation('create', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const payload = MembershipPreferenceMapper.mapInternalToDataverse(
        preferenceData as MembershipPreferenceInternal,
        false,
      );
      const response = await this.dataverseService.request(
        'POST',
        MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME,
        payload,
        credentials,
        app,
      );

      // Map response from Dataverse
      const created = this.mapDataverseToInternal(
        response as MembershipPreferenceDataverse,
      );

      // Preserve lookup GUIDs from original data
      // Note: Dataverse POST response doesn't return lookup fields by default
      // The preferenceData already has GUIDs extracted from @odata.bind by mapper
      if (preferenceData.osot_table_membership_category) {
        created.osot_table_membership_category =
          preferenceData.osot_table_membership_category;
      }
      // XOR: Account OR Affiliate, never both
      if (preferenceData.osot_table_account) {
        created.osot_table_account = preferenceData.osot_table_account;
      }
      if (preferenceData.osot_table_account_affiliate) {
        created.osot_table_account_affiliate =
          preferenceData.osot_table_account_affiliate;
      }

      return created;
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find membership preference by Preference ID (business ID)
   */
  async findByPreferenceId(
    preferenceId: string,
  ): Promise<MembershipPreferenceInternal | null> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = `$filter=${MEMBERSHIP_PREFERENCES_ODATA.PREFERENCE_ID} eq '${preferenceId}'&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preference = responseData?.value?.[0];

      return preference ? this.mapDataverseToInternal(preference) : null;
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find membership preference by GUID
   */
  async findById(id: string): Promise<MembershipPreferenceInternal | null> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = `$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}(${id})?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      return this.mapDataverseToInternal(
        response as MembershipPreferenceDataverse,
      );
    } catch (error) {
      if (error instanceof Error && error.message.includes('404')) {
        return null;
      }
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find membership preferences by membership year
   */
  async findByYear(year: string): Promise<MembershipPreferenceInternal[]> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = `$filter=${MEMBERSHIP_PREFERENCES_ODATA.MEMBERSHIP_YEAR} eq '${year}'&$orderby=${MEMBERSHIP_PREFERENCES_ODATA.CREATED_ON} desc&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preferences = responseData?.value || [];
      return preferences.map((preference: MembershipPreferenceDataverse) =>
        this.mapDataverseToInternal(preference),
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find membership preferences by category lookup
   */
  async findByCategoryId(
    categoryId: string,
  ): Promise<MembershipPreferenceInternal[]> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = `$filter=${MEMBERSHIP_PREFERENCES_ODATA.TABLE_MEMBERSHIP_CATEGORY} eq ${categoryId}&$orderby=${MEMBERSHIP_PREFERENCES_ODATA.CREATED_ON} desc&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preferences = responseData?.value || [];
      return preferences.map((preference: MembershipPreferenceDataverse) =>
        this.mapDataverseToInternal(preference),
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find membership preferences by account lookup
   */
  async findByAccountId(
    accountId: string,
  ): Promise<MembershipPreferenceInternal[]> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = `$filter=${MEMBERSHIP_PREFERENCES_ODATA.ACCOUNT_VALUE} eq ${accountId}&$orderby=${MEMBERSHIP_PREFERENCES_ODATA.CREATED_ON} desc&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preferences = responseData?.value || [];
      return preferences.map((preference: MembershipPreferenceDataverse) =>
        this.mapDataverseToInternal(preference),
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find membership preferences by affiliate lookup
   */
  async findByAffiliateId(
    affiliateId: string,
  ): Promise<MembershipPreferenceInternal[]> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = `$filter=${MEMBERSHIP_PREFERENCES_ODATA.ACCOUNT_AFFILIATE_VALUE} eq ${affiliateId}&$orderby=${MEMBERSHIP_PREFERENCES_ODATA.CREATED_ON} desc&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preferences = responseData?.value || [];
      return preferences.map((preference: MembershipPreferenceDataverse) =>
        this.mapDataverseToInternal(preference),
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find membership preference by user (account or affiliate) and year
   * Business rule: one preference per user per year
   */
  async findByUserAndYear(
    userId: string,
    year: string,
    userType: 'account' | 'affiliate',
  ): Promise<MembershipPreferenceInternal | null> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      // Convert Business ID to GUID
      let userGuid: string;

      if (userType === 'account') {
        const account = await this.dataverseService.request(
          'GET',
          `osot_table_accounts?$filter=osot_account_id eq '${userId}'&$select=osot_table_accountid&$top=1`,
          undefined,
          credentials,
          app,
        );
        const accountData = account as {
          value: Array<{ osot_table_accountid: string }>;
        };
        if (!accountData.value || accountData.value.length === 0) {
          throw new Error(`Account not found: ${userId}`);
        }
        userGuid = accountData.value[0].osot_table_accountid;
      } else {
        const affiliate = await this.dataverseService.request(
          'GET',
          `osot_table_account_affiliates?$filter=osot_affiliate_id eq '${userId}'&$select=osot_table_account_affiliateid&$top=1`,
          undefined,
          credentials,
          app,
        );
        const affiliateData = affiliate as {
          value: Array<{ osot_table_account_affiliateid: string }>;
        };
        if (!affiliateData.value || affiliateData.value.length === 0) {
          throw new Error(`Affiliate not found: ${userId}`);
        }
        userGuid = affiliateData.value[0].osot_table_account_affiliateid;
      }

      // Use OData _value fields for lookups in queries with GUID
      const userField =
        userType === 'account'
          ? MEMBERSHIP_PREFERENCES_ODATA.ACCOUNT_VALUE
          : MEMBERSHIP_PREFERENCES_ODATA.ACCOUNT_AFFILIATE_VALUE;

      const oDataQuery = `$filter=${userField} eq ${userGuid} and ${MEMBERSHIP_PREFERENCES_ODATA.MEMBERSHIP_YEAR} eq '${year}'&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preference = responseData?.value?.[0];

      return preference ? this.mapDataverseToInternal(preference) : null;
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Update membership preference by Preference ID
   */
  async update(
    preferenceId: string,
    updateData: Partial<MembershipPreferenceInternal>,
  ): Promise<MembershipPreferenceInternal> {
    try {
      // First find the record to get the GUID
      const existing = await this.findByPreferenceId(preferenceId);
      if (!existing?.osot_table_membership_preferenceid) {
        throw new Error(
          `Membership preference with ID ${preferenceId} not found`,
        );
      }

      // Use main app credentials for update operations
      const app = getAppForOperation('write', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const payload = MembershipPreferenceMapper.mapInternalToDataverse(
        updateData as MembershipPreferenceInternal,
        true,
      );
      await this.dataverseService.request(
        'PATCH',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}(${existing.osot_table_membership_preferenceid})`,
        payload,
        credentials,
        app,
      );

      // Return the updated data
      return this.mapDataverseToInternal({
        ...existing,
        ...payload,
        osot_table_membership_preferenceid:
          existing.osot_table_membership_preferenceid,
      } as MembershipPreferenceDataverse);
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Update membership preference by GUID
   */
  async updateById(
    id: string,
    updateData: Partial<MembershipPreferenceInternal>,
  ): Promise<MembershipPreferenceInternal> {
    try {
      // Use main app credentials for update operations
      const app = getAppForOperation('write', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const payload = MembershipPreferenceMapper.mapInternalToDataverse(
        updateData as MembershipPreferenceInternal,
        true,
      );
      await this.dataverseService.request(
        'PATCH',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}(${id})`,
        payload,
        credentials,
        app,
      );

      // Return updated record
      const updated = await this.findById(id);
      if (!updated) {
        throw new Error('Failed to retrieve updated membership preference');
      }

      return updated;
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Delete membership preference by Preference ID (hard delete)
   */
  async delete(preferenceId: string): Promise<void> {
    try {
      // First find the record to get the GUID
      const existing = await this.findByPreferenceId(preferenceId);
      if (!existing?.osot_table_membership_preferenceid) {
        throw new Error(
          `Membership preference with ID ${preferenceId} not found`,
        );
      }

      // Use main app credentials for delete operations
      const app = getAppForOperation('delete', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      // Hard delete
      await this.dataverseService.request(
        'DELETE',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}(${existing.osot_table_membership_preferenceid})`,
        undefined,
        credentials,
        app,
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Delete membership preference by GUID (hard delete)
   */
  async deleteById(id: string): Promise<void> {
    try {
      // Use main app credentials for delete operations
      const app = getAppForOperation('delete', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      // Hard delete
      await this.dataverseService.request(
        'DELETE',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}(${id})`,
        undefined,
        credentials,
        app,
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * List all membership preferences with pagination
   */
  async findAll(options?: {
    skip?: number;
    top?: number;
    orderBy?: string;
  }): Promise<MembershipPreferenceInternal[]> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const skip = options?.skip || 0;
      const top = options?.top || 50;
      const orderBy =
        options?.orderBy || `${MEMBERSHIP_PREFERENCES_ODATA.CREATED_ON} desc`;

      const oDataQuery = `$skip=${skip}&$top=${top}&$orderby=${orderBy}&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preferences = responseData?.value || [];
      return preferences.map((preference: MembershipPreferenceDataverse) =>
        this.mapDataverseToInternal(preference),
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Count total membership preferences
   */
  async count(): Promise<number> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = '$count=true&$top=1';
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as { '@odata.count'?: number };
      return responseData?.['@odata.count'] || 0;
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Check if a preference exists for user and year (uniqueness validation)
   */
  async existsByUserAndYear(
    userId: string,
    year: string,
    userType: 'account' | 'affiliate',
  ): Promise<boolean> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      // Use OData _value fields for lookups in queries
      const userField =
        userType === 'account'
          ? MEMBERSHIP_PREFERENCES_ODATA.ACCOUNT_VALUE
          : MEMBERSHIP_PREFERENCES_ODATA.ACCOUNT_AFFILIATE_VALUE;

      const filterQuery = `${userField} eq ${userId} and ${MEMBERSHIP_PREFERENCES_ODATA.MEMBERSHIP_YEAR} eq '${year}'`;
      const oDataQuery = `$filter=${filterQuery}&$count=true&$top=1`;

      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as { '@odata.count'?: number };
      return (responseData?.['@odata.count'] || 0) > 0;
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Find preferences with auto-renewal enabled
   */
  async findByAutoRenewal(
    autoRenewal: boolean,
  ): Promise<MembershipPreferenceInternal[]> {
    try {
      // Use main app credentials for read operations
      const app = getAppForOperation('read', 'main');
      const credentials = this.dataverseService.getCredentialsByApp(app);

      const oDataQuery = `$filter=${MEMBERSHIP_PREFERENCES_ODATA.AUTO_RENEWAL} eq ${autoRenewal}&$orderby=${MEMBERSHIP_PREFERENCES_ODATA.CREATED_ON} desc&$select=${MEMBERSHIP_PREFERENCES_ODATA.SELECT_FIELDS}`;
      const response = await this.dataverseService.request(
        'GET',
        `${MEMBERSHIP_PREFERENCES_ODATA.TABLE_NAME}?${oDataQuery}`,
        undefined,
        credentials,
        app,
      );

      const responseData = response as {
        value?: MembershipPreferenceDataverse[];
      };
      const preferences = responseData?.value || [];
      return preferences.map((preference: MembershipPreferenceDataverse) =>
        this.mapDataverseToInternal(preference),
      );
    } catch (error) {
      throw new Error(
        `${ErrorMessages[ErrorCodes.GENERIC].publicMessage}: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  }

  /**
   * Transform raw Dataverse data to internal format
   */
  mapDataverseToInternal(
    dataverseData: MembershipPreferenceDataverse,
  ): MembershipPreferenceInternal {
    return MembershipPreferenceMapper.mapDataverseToInternal(dataverseData);
  }

  /**
   * Transform internal data to Dataverse format
   */
  mapInternalToDataverse(
    internalData: Partial<MembershipPreferenceInternal>,
  ): Partial<MembershipPreferenceDataverse> {
    return MembershipPreferenceMapper.mapInternalToDataverse(
      internalData as MembershipPreferenceInternal,
      true,
    );
  }
}
