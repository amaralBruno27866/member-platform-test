/**
 * OTA Education Category Scheduler Controller
 *
...
import {
  ...
  CategoryUpdateStats,
} from '../schedulers/ota-education-category.scheduler';
...
/**
 * Admin Controller for OTA Education Category Scheduler
 *
 ...
 */
@Controller('admin/ota-education/scheduler')
@ApiTags('Admin OTA Education Scheduler')
@ApiBearerAuth('JWT-auth')
@UseGuards(AuthGuard('jwt'))
export class OtaEducationCategorySchedulerController {
  private readonly logger = new Logger(
    OtaEducationCategorySchedulerController.name,
  );

  constructor(
    private readonly categoryScheduler: OtaEducationCategoryScheduler,
  ) {}
  ...
  async triggerManualUpdate(
    ...
    try {
      const result = await this.categoryScheduler.triggerManualUpdate(false);
  ...
  getCategoryStats() {
    ...
    try {
      // For now, we'll return basic placeholder statistics
      // In the future, this could be enhanced with actual category counting from the repository
      const stats = {
        students: 0,
        newGraduated: 0,
        graduated: 0,
      ...
      this.logger.error(
        `Error retrieving education category statistics - Operation: ${operationId}`,
        error,
      );
