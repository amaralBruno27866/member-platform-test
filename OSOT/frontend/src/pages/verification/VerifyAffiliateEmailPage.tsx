...
import { verifyAffiliateEmail } from '../../services/verificationService';
...
export function VerifyAffiliateEmailPage() {
  ...
  useEffect(() => {
    const processVerification = async () => {
      ...
      try {
        const response = await verifyAffiliateEmail(sessionId, token);
        
        if (response.success) {
          setState('success');
          setMessage(
            response.message || 
            'Email verified successfully! Your affiliate organization registration is now pending admin approval. You will receive an email once your organization is approved.'
          );
        } else {
          setState('error');
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            {state === 'loading' && 'Please wait while we verify your organization email...'}
            {state === 'success' && 'Your organization email has been successfully verified'}
