/**
 * Update Audience Target DTO
 * Simplified DTO for audience target updates - all fields optional for partial updates
 *
 * PHILOSOPHY:
 * - All fields are optional for partial update support
 * - Product relationship can be changed (enforces one-to-one constraint)
 * - System fields (ID, timestamps, owner) are NOT editable
 * - Each field allows 0-50 selections
 *
 * USAGE CONTEXT:
 * - PATCH /private/audience-target/:id (update existing target)
 * - Supports partial updates (only provided fields are updated)
 * - Repository validates product relationship if changed
 *
 * BUSINESS RULES:
 * - All fields optional for partial update support
 * - Product change validated against one-to-one constraint
 ...
 */
export class UpdateAudienceTargetDto {
  // ========================================
  ...
  @ApiProperty({
    ...
    description:
      'Product relationship in OData bind format (optional) - Format: /osot_table_products/{productGuid}',
    required: false,
  })
  @IsOptional()
  ...
  @ApiProperty({
    ...
    description:
      'Account groups to target (multi-select, optional, 0-50 selections). Send null or [] to clear.',
    enum: AccountGroup,
    isArray: true,
    required: false,
    nullable: true,
  ...
  @IsOptional()
  @ValidateIf(
    ({ osot_account_group }: Partial<UpdateAudienceTargetDto>) =>
      osot_account_group !== null,
  )
  ...
  osot_account_group?: AccountGroup[] | null;
